...
def Get_Flags_list(path_to_json: Path | str) -> tuple[dict[str, str], dict[str, int]]:
    ...
    if not path.exists():
        ...
        print("Using default path:", path , "Default path does not exist" , "Using empty flags list")
        return flags, points
...
flags, flag_points = Get_Flags_list("C:\\Users\\User\\source\\Projects\\Reverse-Engineering-CTF\\flags.json")
print(f"Loaded {len(flags)} flags from JSON.")
print("Flags loaded:", flags)
earned_points = 0
